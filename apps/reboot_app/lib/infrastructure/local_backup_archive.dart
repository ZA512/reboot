...
/// SQLite3MultipleCiphers implementation of the recovery archive boundary.
final class LocalBackupArchiveService implements LocalBackupArchive {
  const LocalBackupArchiveService({
  ...
  /// Copies the current immutable journal into a new independently keyed file.
  @override
  Future<PreparedLocalBackup> prepare(LocalRebootService service) async {
    ...
    final recoveryCode = LocalBackupRecoveryCode.encode(keyBytes);
    final key = EncryptedDatabaseKey(keyBytes);
    keyBytes.fillRange(0, keyBytes.length, 0);
    LocalEventJournal? archive;
    var completed = false;
    try {
      archive = await openJournal(file.absolute.path, key);
      await archive.appendAll([for (final entry in snapshot) entry.event]);
      await archive.close();
      archive = null;
  ...
  /// Authenticates and fully replays an archive before returning its events.
